...
public class majWidget {
	...
	private void majBddProduitPerime(Context p_context) {
		...
		if (nbDenregistrement > 0) {
			...
			// on commence par recalculer la date de permeption suite au bug de
			// calcul lors de l'entrée du produit:
			for (int j = 0; j < nbDenregistrement; j++) {
				int total = (100 * j) / nbDenregistrement;
				System.out.println("Pourcentage: " + total
						+ " maj bdd produit perime dans maj Widget");
