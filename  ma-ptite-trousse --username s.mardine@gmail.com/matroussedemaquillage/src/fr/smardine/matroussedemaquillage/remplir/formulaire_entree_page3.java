...
public class formulaire_entree_page3 extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		 ...
		 * @Override public void afterTextChanged(Editable s) { String str = nomProduit.getText().toString(); if(!str.equals("")){ String
		 * str1 = UpperCaseWords(str); nomProduit.setText(""+str1+""); } }
		 * @Override public void beforeTextChanged(CharSequence arg0, int arg1, int arg2, int arg3) { String str =
		 * nomProduit.getText().toString(); len = str.length(); }
		...
		CharSequence[] items = { "Ajouter un produit", "Dupliquer un produit", "Revenir à la page d'accueil" };
		...
		/*
		 * Animlineaire anim = new Animlineaire (false,true); mDateDisplay.startAnimation(anim.getAnim());
		 * mPickDate.startAnimation(anim.getAnim()); nomProduit.startAnimation(anim.getAnim()); numeroTeinte.startAnimation(anim.getAnim());
		 * dureeVie.startAnimation(anim.getAnim()); BoutonValider3.startAnimation(anim.getAnim());
		 ...
		 * "Fond  de teint(6 à 12 mois )","Blush-Poudre (12 à 18 mois)","Fards (12 à 18 mois)","Crayons (12 à 36 mois)",
		 * "Rouge à lèvres (12 à 36 mois)", "Pinceau (nettoyage tout les mois)","Vernis à ongle (Voir sur la bouteille)"}; /*
		 * adAide.setSingleChoiceItems(items , -1, new DialogInterface.OnClickListener() { public void onClick(DialogInterface dialog, int
		 * item){ /* User clicked on a radio button do some stuff
		 */
		/*
		 * if (item==0){//Mascara NbMoisChoisiBas=3; NbMoisChoisiHaut=6; }if (item==1){//fond de teint NbMoisChoisiBas=6;
		 * NbMoisChoisiHaut=12; } if (item==2){//blush-poudres NbMoisChoisiBas=12; NbMoisChoisiHaut=18; } if (item==3){//fards
		 * NbMoisChoisiBas=12; NbMoisChoisiHaut=18; } if (item==4){//crayons NbMoisChoisiBas=12; NbMoisChoisiHaut=36; } if
		 * (item==5){//Rouges à lèvres NbMoisChoisiBas=12; NbMoisChoisiHaut=36; } if (item==6){//pinceaux NbMoisChoisiBas=1;
		 * NbMoisChoisiHaut=1; } if (item==7){//Vernis NbMoisChoisiBas=6; NbMoisChoisiHaut=12; } } });
		 */
	...
	/*
	 * public String UpperCaseWords(String line){ line = line.trim().toLowerCase(); String data[] = line.split("\\s"); line = ""; for(int i
	 * =0;i< data.length;i++) { if(data[i].length()>1) { line = line + data[i].substring(0,1).toUpperCase()+data[i].substring(1)+" "; } else
	...
	protected void insereProduitDansTableEtPrposeLaSuite() {
		...
		// recuperation des infos dans la table temporaires et insertion dans la table produit finale.
		ArrayList[] Categorie_Cochée = objBd.renvoiCategorieEtProduitCochée();

		String[] Colonnes = { "nom_marque_choisie", "nom_produit", "numero_Teinte", "DateAchat", "Duree_Vie" };
		ArrayList[] trousse_tempo = objBd.renvoi_liste_ValeurTroussetempo("trousse_tempo", Colonnes);
		...
		Date DatePeremption = getDateAfterDays(DateAchatEnMilli, nbJours);// on calcule la date de permetpion en fonction de la date
		...
		insertOk = objBd.InsertDonnéedansTable("produit_Enregistre", valuesProduitsFinal);
		if (insertOk) {
			popUp("Insert Ok");
			String Table = "trousse_produits";
			ContentValues modifiedValues = new ContentValues();
	...
	protected void onResume() {
		...
		// updateDisplay();
		// } else {
		// mDateDisplay.setText(new StringBuilder().append("Date choisie: ").append(DateAchat));
		// }
		// if (!nomDuProduit.equals("")) {
		// nomProduit.setText(nomDuProduit);
		// }
