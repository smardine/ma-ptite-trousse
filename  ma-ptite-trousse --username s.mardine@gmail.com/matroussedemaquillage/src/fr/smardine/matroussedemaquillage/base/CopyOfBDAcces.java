...
public class CopyOfBDAcces {
	...
	public CopyOfBDAcces(Context ctx, Handler mHandler) {
	 
		 this.mCtx = ctx;
		 CopyOfBDAcces.handler = mHandler;
...
private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			try {
				...
				for (int i=0;i<listeScripts.size();i++){
					Script = listeScripts.get(i).toString();
		...
		/*	public int updateTable(String Table, ContentValues Value,String whereClause,String[] whereArgs){
					int nbdeChampAffecté = mDb.update(Table, Value, whereClause, whereArgs);
	...
	public ArrayList[] renvoi_liste_produits(String Catégorie){
		  ...
		  ArrayList[] aTableRetour = new ArrayList[25];
	...
	/* Check if our result was valid. */
     if (objCursor != null) {            	 
    	 for (int i=0;i<itotal;i++){
           ...
           if (!resultDatePeremp.equals(null)){
...
public ArrayList[] renvoi_liste_TrousseTempo(){
  ...
  return aTableRetour;
    ...
    public int majTable(String Table, ContentValues modifiedValues,	
