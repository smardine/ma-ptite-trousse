...
import fr.smardine.matroussedemaquillage.base.G_remplirBase;
		  ...
		  for(int i=0;i<nbdeDateEnregistré;i++){
			  ...
			  if (datePerem[0].get(i)!=null){
				  ...
				  if (nbJour<=0){//on a deja depassé la date limite
					  	/*String Message = "DateDepassée sur l'id produit n° "+datePerem[1].get(i);
						popUp(Message);*/
...
private void CalculDatePeremtionEtMajDansBase(String dateAchat1,String dureeVie,String idProduit) throws Exception{
	...
	int DureeVie=0;
		...
		Date DatePeremption1 = getDateAfterDays(DateAchatEnMilli,nbJours);// on calcule la date de permetpion en fonction de la date d'achat+nb de jour donné par l'utilisateur
